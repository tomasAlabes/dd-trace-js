...
const DATADOG_TEST_OPTIMIZATION_BOOTSTRAPS = new Set([
  ...
  'dd-trace/ci/register',
  'dd-trace/ci/register.js',
  'dd-trace/ci/init',
  'dd-trace/ci/init.js',
])
...
function configureMainProcessExecutionHooks (
  ...
  addSetupFileToVitestConfigs(ctx, setupFile)
  setVitestWorkerSetupEnvironment(ctx, {
    DD_TEST_OPT_VITEST_ATTEMPT_TO_FIX_RETRIES: String(testManagementAttemptToFixRetries),
    DD_TEST_OPT_VITEST_ATTEMPT_TO_FIX_TESTS: JSON.stringify(attemptToFixTests || {}),
    DD_TEST_OPT_VITEST_DISABLED_TESTS: JSON.stringify(disabledTests || {}),
    DD_TEST_OPT_VITEST_EFD_ENABLED: shouldConfigureEarlyFlakeDetection ? '1' : '0',
    DD_TEST_OPT_VITEST_EFD_RETRIES: String(
      getConfiguredEfdRetryCount(earlyFlakeDetectionSlowTestRetries, earlyFlakeDetectionNumRetries)
    ),
    DD_TEST_OPT_VITEST_EFD_SLOW_RETRIES: JSON.stringify(earlyFlakeDetectionSlowTestRetries),
    DD_TEST_OPT_VITEST_KNOWN_TESTS: JSON.stringify(knownTests?.vitest || {}),
    DD_TEST_OPT_VITEST_REPOSITORY_ROOT: testSessionConfiguration.repositoryRoot || process.cwd(),
  })
}

function getVitestConfigs (ctx) {
  const configs = new Set([ctx.config])
...
function addSetupFileToVitestConfigs (ctx, setupFile) {
  const configs = getVitestConfigs(ctx)
...
function setVitestWorkerSetupEnvironment (ctx, envValues) {
  for (const [name, value] of Object.entries(envValues)) {
    // eslint-disable-next-line eslint-rules/eslint-process-env
    process.env[name] = value
  }

  for (const config of getVitestConfigs(ctx)) {
    if (!config) continue
    config.env = {
      ...config.env,
      ...envValues,
...
function installMainProcessReporter (
  ...
  ctx.reporters.push(createMainProcessReporter(frameworkVersion, testSessionConfiguration, testOptimizationData))
}

function createMainProcessReporter (frameworkVersion, testSessionConfiguration = {}, testOptimizationData = {}) {
    ...
    onFinished (files) {
      ...
      for (const file of files) {
        if (!finishedTestModules.has(file.id)) {
          reportTestModule(createTestModuleFromFile(file))
        }
      }
    },
  }
