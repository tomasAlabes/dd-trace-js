...
const attemptToFixTests = parseTests('DD_TEST_OPT_VITEST_ATTEMPT_TO_FIX_TESTS')
const attemptToFixRetries = Number(process.env.DD_TEST_OPT_VITEST_ATTEMPT_TO_FIX_RETRIES || 0)
const disabledTests = parseDisabledTests()
const earlyFlakeDetectionRetries = Number(process.env.DD_TEST_OPT_VITEST_EFD_RETRIES || 0)
const earlyFlakeDetectionSlowRetries = parseTests('DD_TEST_OPT_VITEST_EFD_SLOW_RETRIES')
...
const isEarlyFlakeDetectionEnabled = isTrue(process.env.DD_TEST_OPT_VITEST_EFD_ENABLED)
const knownTests = parseTests('DD_TEST_OPT_VITEST_KNOWN_TESTS')
const repositoryRoot = realpath(process.env.DD_TEST_OPT_VITEST_REPOSITORY_ROOT || process.cwd())
...
function parseDisabledTests () {
  return parseTests('DD_TEST_OPT_VITEST_DISABLED_TESTS')
}

function parseTests (envName) {
  try {
